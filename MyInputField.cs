...
using UnityEngine;
        ...
        ICanvasElement
    {
        ...
        private int DetermineCharacterLine(int charPos, TextGenerator generator)
        {
            ...
            //某种错误下generator.lineCount与generator.lines.Count可能不一样，已修改防止越界，2017.3
...
//                     UnityEngine.Debug.LogError("str:" + processed);
//                     UnityEngine.Debug.LogError("before:" + cachedInputTextGenerator.ToString() + ", lineCount and lines:" + cachedInputTextGenerator.lineCount + ", " + cachedInputTextGenerator.lines.Count);
                    if (cachedInputTextGenerator.lineCount != cachedInputTextGenerator.lines.Count)
                    {
                        ...
                        //但Populate函数的参数必须与上次传入的不同才会重新执行，所以此处删除最后一个字。由于此帧中UpdateLabel还会被调用，所以删除一个字又会被纠正回来
...
//                    UnityEngine.Debug.LogError("after:" + cachedInputTextGenerator.ToString() + ", lineCount and lines:" + cachedInputTextGenerator.lineCount + ", " + cachedInputTextGenerator.lines.Count);
        ...
        {
            if (gen.lines.Count <= 0 || line < 0) return 0;     //line可能传入-1，防止越界 2017.3
        ...
        private void SetDrawRangeToContainCaretPosition(TextGenerator gen, int caretPos, ref int drawStart, ref int drawEnd)
        {
            ...
            if (multiLine)
            {
                ...
                else
                {
                    ...
                    //startLine可能为-1，防止越界 2017.3
                    if (endLine < 0 || startLine < 0 || endLine >= lines.Count)
                    {
                        //UnityEngine.Debug.LogError("InputField bug   为防止越界，已做保护");
                        return;
                    }
        ...
        private void GenerateCursor(List<UIVertex> vbo, Vector2 roundingOffset)
        {
            ...
            m_CursorVerts[0].position = UnityExtension.Vector3Ex(startPosition.x, startPosition.y - height, 0.0f);
            m_CursorVerts[1].position = UnityExtension.Vector3Ex(startPosition.x + width, startPosition.y - height, 0.0f);
            m_CursorVerts[2].position = UnityExtension.Vector3Ex(startPosition.x + width, startPosition.y, 0.0f);
            m_CursorVerts[3].position = UnityExtension.Vector3Ex(startPosition.x, startPosition.y, 0.0f);
        ...
        private void GenerateHightlight(List<UIVertex> vbo, Vector2 roundingOffset)
        {
            ...
            while (currentChar <= endChar && currentChar < gen.characterCountVisible)
            {
                if (currentChar + 1 == nextLineStartIdx || currentChar == endChar)
                {
                    ...
                    vert.position = UnityExtension.Vector3Ex(startPosition.x, endPosition.y, 0.0f) + (Vector3)roundingOffset;
                    vbo.Add(vert);

                    vert.position = UnityExtension.Vector3Ex(endPosition.x, endPosition.y, 0.0f) + (Vector3)roundingOffset;
                    vbo.Add(vert);

                    vert.position = UnityExtension.Vector3Ex(endPosition.x, startPosition.y, 0.0f) + (Vector3)roundingOffset;
                    vbo.Add(vert);

                    vert.position = UnityExtension.Vector3Ex(startPosition.x, startPosition.y, 0.0f) + (Vector3)roundingOffset;
                    vbo.Add(vert);
